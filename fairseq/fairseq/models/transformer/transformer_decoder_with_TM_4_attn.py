...
class TransformerDecoderBaseWithTM(FairseqIncrementalDecoder):
    ...
    def forward(
        ...
        if not with_TM_ensemble:
            ...
            # clone_attn_normalized_all = torch.stack(attn_all, dim=0)  # TM_num x T x B x C
            # clone_attn_normalized_all = torch.stack(learnable_p_attn_norm_all, dim=0)  # TM_num x T x B x C
