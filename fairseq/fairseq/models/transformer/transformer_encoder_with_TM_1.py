...
class TransformerEncoderBaseWithTM(FairseqEncoder):
    ...
    def forward_scriptable(
        ...
        memory_encoder_padding_mask_chunk = memory_encoder_padding_mask.chunk(TM_num, dim=1)  # TM_num x T x B
        memory_encoder_padding_mask = torch.cat(memory_encoder_padding_mask_chunk, dim=0)  # T*TM_num x B
        # memory_encoder_padding_mask = memory_encoder_padding_mask.contiguous().view(-1, bsz)  # T x B*TM_num -> T*TM_num x B
    ...
    def reorder_encoder_out(self, encoder_out: Dict[str, List[Tensor]], new_order):
        ...
        if len(encoder_out["TMs_similarity"]) == 0:
            new_TMs_similarity = []
        else:
            new_TMs_similarity = [
