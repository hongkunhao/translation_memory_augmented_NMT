...
def make_dataset(path, impl, fix_lua_indexing=False, dictionary=None):
    ...
    elif impl == "raw_float" and IndexedRawFloatDataset.exists(path):
        # print("huffman\n\n")
        return IndexedRawFloatDataset(path)
    return None
...
class IndexedRawFloatDataset(FairseqDataset):
    """Takes a text file original lines as input and binarizes it in memory at instantiation."""
    def __init__(self, path):
        self.tokens_list = []
        self.lines = []
        self.sizes = []
        self.is_one_float_per_line = True
        self.read_data(path)
        self.size = len(self.tokens_list)
    ...
    def read_data(self, path):
        with open(path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                nwords = len(words)
                if nwords > 1:
                    self.is_one_float_per_line = False
                floats = torch.FloatTensor(nwords)
                for i, word in enumerate(words):
                    cur_float = float(word)
                    floats[i] = cur_float
                self.tokens_list.append(floats)
                self.sizes.append(len(floats))
        self.sizes = np.array(self.sizes)
